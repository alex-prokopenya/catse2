...
namespace ClickAndTravelSearchEngine.ExcursionSearchExt
{
    public class VizitExcSearcher
    {
        ...
        public VizitExcursionVariant[] SearchExcursionsVizit(int cityId, DateTime beginDate, DateTime endDate, int nMen)
        {
            string selectQuery = " select CS_DATE, CS_DATEEND, CS_RATE, isnull(cs_type,0) as cs_type, CS_COSTNETTO, CS_COST, CS_WEEK, ED_NAME, ED_StdKey, tr_key, ED_KEY" +
            ...
            while (reader.Read())
            {
                ...
                string edKey = reader["ed_key"].ToString();
                string trKey = reader["tr_key"].ToString();

                //для всех дат проверить, есть ли экскурсия
                string key = string.Format("{0}*{1}*{2}*{3}*{4}", stdKey, edKey, trKey, price, rateCode);
            ...
            if (dictionary.Count == 0) return new VizitExcursionVariant[0];

            return ConvertToExcVariantsVizit(dictionary);
        }
        ...
        private VizitExcursionVariant[] ConvertToExcVariantsVizit(Dictionary<string, List<DateTime>> dictionary)
        {
            ...
            var respList = new List<VizitExcursionVariant>();
            //проходимся по списку и формируем ответ
            foreach (string key in dictionary.Keys)
            {
                if (dictionary[key].Count == 0) continue;

                //делим ключ, чтобы получить инфу об экскурсии
                var part = key.Split('*');
                if (part[4] == "E") part[4] = "EUR";
                ...
                respList.Add(
                                new VizitExcursionVariant()
                                {
                                    DescriptionId = Convert.ToInt32(part[0]),
                                    TransportId = Convert.ToInt32(part[2]),
                                    Id = Convert.ToInt32(part[1]),
                                    Prices = MtHelper.ApplyCourses(Convert.ToDecimal(part[3]), _courses)
                                }
                );
            }
