...
{
    public class ClickExcSearcher
     {
        private int sourcePack = Convert.ToInt32(ConfigurationManager.AppSettings["ClickExcursionSourcePack"]);

        private SqlConnection _myConnection = new SqlConnection(ConfigurationManager.AppSettings["ClickConnectionString"]);

        private Dictionary<int, int> citiesLib = new Dictionary<int, int>();

        public ClickExcSearcher() 
        {
            #region Чехия
            ...
            //Италия
            citiesLib.Add(1238, 446);//рим
            citiesLib.Add(1226, 706);//венеция
            citiesLib.Add(1244, 962);//флоренция
            citiesLib.Add(1233, 723);//милан
            citiesLib.Add(1224, 963);//болонья  
            citiesLib.Add(1711, 965);//парма    
            citiesLib.Add(3187, 964);//равенна  
            citiesLib.Add(1710, 966);//модена   
            citiesLib.Add(3512, 959);//неаполь  
            citiesLib.Add(1227, 918);//верона   
        ...
        public JsonArray GetExcursionCalendar(int excursionId, DateTime MinDate, DateTime MaxDate)
        {
            return null;
        }
        ...
        public ExcursionVariant[] SearchExcursions(int cityId, DateTime beginDate, DateTime endDate, int nMen)
        {
            ...
            string selectQuery = " select CS_DATE, CS_DATEEND,CS_RATE, isnull(cs_type,0) as cs_type, CS_COSTNETTO, CS_COST, CS_WEEK, ED_NAME, [ct_desckey] as ED_StdKey " +
                                 " FROM [dbo].tbl_Costs, dbo.ExcurDictionary as a, dbo.[Transport] as c, dbo.[excursionLinks] as b " +
                                 " where cs_code=b.[ed_key] and cs_subcode1=b.[tr_key]  and CS_SVKEY = 4 and " +
                                 ...
                                 " CS_CODE = a.ED_KEY and ED_CTKEY = {2} and CS_PKKEY = {3} " +
                                 (nMen > 1? " and not ED_NAME like '%sgl%'":"") +
                                 " and cs_subcode1 = c.tr_key and isnull(tr_nmen,100) >= " + nMen +
            ...
            Helpers.Logger.WriteToLog("click: " + selectQuery);
            ...
            while (reader.Read())
            {
                ...
                Helpers.Logger.WriteToLog("itemcost");
            }
