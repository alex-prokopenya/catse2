...
namespace ClickAndTravelSearchEngine.MasterTour
{
    public class MtHelper
    {
        ...
        public static int SaveFlightToCache(string[] turists_ids, SF_service.Flight ticket, string ticket_id) 
        {
            ...
            try
            {
                SqlCommand com = new SqlCommand(String.Format("insert into [CATSE_Flights] ([ft_ticketid],[ft_route],[ft_date] ,[ft_price],[ft_turists]) OUTPUT INSERTED.ft_id " +
                                                        "VALUES ('{0}','{1}','{2}',{3},'{4}')", AntiInject(ticket_id),
                                                                                             "fake route",//   ticket.RouteItems[0].Legs[0].DepartureCode + " - " + ticket.RouteItems[0].Legs[0].ArrivalCode,
